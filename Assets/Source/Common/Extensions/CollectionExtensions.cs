...
namespace Quinn
{
	public static class CollectionExtensions
	{
		...
		/// <returns>A copy in randomized order.</returns>
		public static IEnumerable<T> Randomized<T>(this IEnumerable<T> collection)
		{
			var randomized = new List<T>();
			var all = new List<T>(collection);

			while (all.Count > 0)
			{
				var selected = all.GetRandom();
				randomized.Add(selected);
				all.Remove(selected);
			}
		...
		public static T GetWeightedRandom<T>(this IEnumerable<T> collection, System.Func<T, float> getWeight, bool reverse = false)
		{
			if (!collection.Any())
			{
				return default;
			...
			float sum = collection.Sum(x => getWeight(x));

			foreach (var item in collection)
			{
				if (reverse)
				{
					if (Random.value <= (1f - (getWeight(item) / sum)))
					{
						return item;
					}
				}
				...
				{
					if (Random.value <= getWeight(item) / sum)
					{
						return item;
					}
				}
